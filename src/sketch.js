...
import { aspectRatioMaxClamp, aspectRatioMinClampRatio, containAspectRatio, coverAspectRatio, } from './algorithms';
import { getStreamMaxDimensionsConstraints, VideoCamera, } from './video-camera';
...
export const sketch = (p5) => {
    ...
    p5.draw = () => {
        ...
        // console.log('w,h: ' + w + ',' + h);
        let margin = [(window.innerWidth - w) / 2, (window.innerHeight - h) / 2];
    ...
    function drawBoundingBox(p5, w, h, margin) {
        p5.resetMatrix();
        p5.translate(...margin);
        p5.noFill();
    ...
    function drawPixels(p5, pixels) {
        ...
        for (let i = 0; i < w; i++) {
            for (let j = 0; j < h; j++) {
                ...
                p5.noStroke();
                if (gradient)
                    p5.fill(avg, avg, avg, a);
                else if (color)
                    p5.fill(r, b, g, a);
                let x = i * pixel_size + x_translate, y = j * pixel_size + y_translate;
    ...
    function drawRawVideo(p5) {
        ...
        let vw = video.width * ratio, vh = video.height * ratio;
        ...
        console.log('x_translate: ' + x_translate.toFixed(2) + ', y-translate: ' + y_translate.toFixed(2));
        // p5.scale(ratio);
        console.log('bout to draw');
        console.log(video.tag);
        p5.image(video, x_translate, y_translate, vw, vh);
    }
    function accountForScale(v1, v2, ratio) {
        return (v2 / ratio - v1) / 2;
    }
    function drawVideo(p5, _w, _h) {
        cropVideo(stream, 0);
        p5.clear(0, 0, 0, 0);
        if (black)
            p5.background(0);
        ...
        p5.scale(ratio);
        let pixels = chunk(video.pixels, 4);
        if (black)
            p5.fill(255, 255, 255);
        else
            p5.fill(0, 0, 0);
        ...
        for (let i = 0; i < video.width; i++)
            for (let j = 0; j < video.height; j++) {
                ...
                if (gradient)
                    p5.fill(avg, avg, avg);
                let x = i * pixel_size, y = j * pixel_size;
                // p5.square(x, y, pixel_size);
